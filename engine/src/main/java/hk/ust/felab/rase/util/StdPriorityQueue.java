 ...
 */
public class StdPriorityQueue<E> extends AbstractQueue<E> implements
	 ...
	 */
	public StdPriorityQueue(int initialCapacity) {
	 ...
	 */
	public StdPriorityQueue(Collection<? extends E> c) {
		initFromCollection(c);
		if (c instanceof SortedSet)
			comparator = (Comparator<? super E>) ((SortedSet<? extends E>) c)
					.comparator();
		else if (c instanceof StdPriorityQueue)
			comparator = (Comparator<? super E>) ((StdPriorityQueue<? extends E>) c)
					.comparator();
		else {
			comparator = null;
			heapify();
	 ...
	 *             null
	 */
	public StdPriorityQueue(StdPriorityQueue<? extends E> c) {
	 ...
	 */
	public StdPriorityQueue(SortedSet<? extends E> c) {
		comparator = (Comparator<? super E>) c.comparator();
		initFromCollection(c);
	}
	...
	private final class Itr implements Iterator<E> {
		...
		public void remove() {
			if (expectedModCount != modCount)
				throw new ConcurrentModificationException();
			if (lastRet != -1) {
				E moved = StdPriorityQueue.this.removeAt(lastRet);
				lastRet = -1;
				if (moved == null)
					cursor--;
				else {
				...
				StdPriorityQueue.this.removeEq(lastRetElt);
				lastRetElt = null;
			} else {
